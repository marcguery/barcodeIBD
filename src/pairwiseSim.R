...
pairwise.edges.all <- melt(dist.dna(barcodes.dnabin, model = "raw",
                              pairwise.deletion = T,
                              as.matrix = T))
...
colnames(pairwise.edges.all) <- c("ID1", "ID2", "pairwise")
pairwise.edges <- merge(pairwise.edges,
                        barcodes.comp.num)
##########################################
#####################SAVE DATA#####################
write.table(pairwise.edges,paste0(outDir, "/barcode-pairwise.tsv"),quote = F,row.names = F)
##########################################
